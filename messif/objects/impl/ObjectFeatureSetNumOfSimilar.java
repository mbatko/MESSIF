...
public class ObjectFeatureSetNumOfSimilar extends ObjectFeatureSet {
    ...
    public ObjectFeatureSetNumOfSimilar(String locatorURI, Collection<? extends ObjectFeature> objects) {
...
//    }
//
//    public ObjectFeatureSetNumOfSimilar (ObjectFeatureOrderedSet superSet, float minX, float maxX, float minY, float maxY) {
//        super (superSet, minX, maxX, minY, maxY);
//    }
//
//    public ObjectFeatureSetNumOfSimilar (ObjectFeatureOrderedSet superSet) {
//        super (superSet);
//    }
    ...
    public ObjectFeatureSetNumOfSimilar(BufferedReader stream, Map<String, ? extends Serializable> additionalParameters) throws IOException {
...
//    }
    ...
    /** Default cost */
    public static SequenceMatchingCost cost = new SequenceMatchingCost();
    ...
    protected float getDistanceImpl(LocalAbstractObject o, float distThreshold) {
        ...
        for (ObjectFeature f2 : obj.objects) {
            for (ObjectFeature f1 : this.objects) {
                float c = cost.getCost(f1, f2);
                if (c == cost.getMatchExact())
                    numExact++;
                else if (c >= cost.getMatchApprox() && c < cost.getMatchMismatch())
                    numApprox++;
            }
        }
