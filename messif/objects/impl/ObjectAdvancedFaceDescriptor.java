 ...
 * Connection connection = DriverManager.getConnection("jdbc:mysql://andromeda/fdrdata", "user", "pass");
 * PreparedStatement sqlCmd = connection.prepareStatement("SELECT concat('#objectKey messif.objects.AbstractObjectKey ', id, '\n', ard)  FROM faces");
 * ResultSet rs = sqlCmd.executeQuery();
 * while (rs.next())
...
public class ObjectAdvancedFaceDescriptor extends LocalAbstractObjectAutoImpl {
    ...
    /** Weights for the metric function */
